...
async def handle_message(message):
    ...
    if conversation_stage == "get_patient_id":
        patient_id = message.content.strip()

        # Check if the input is exactly 8 digits
        if not re.fullmatch(r"\d{8}", patient_id):
            await cl.Message("❌ Patient ID must be exactly 8 digits (e.g., 46581382). Please try again:").send()
            return  # Stay in "get_patient_id" stage

        claim_info["patient_id"] = patient_id
    ...
    if conversation_stage == "get_treatment_code":
        treatment_code = message.content.strip()

        # Allow "ignore" to skip validation and proceed
        if treatment_code.lower() != "ignore":
            # Validate only if not "ignore"
            if not re.fullmatch(r"[A-Z]\d{2}\.\d{1,2}|\d{5}", treatment_code, re.IGNORECASE):
                await cl.Message(
                    "❌ Treatment code must be like `M54.5`, `Z12.31`, `83036`, or type `ignore` to skip. Please try again:").send()
                return  # Stay in "get_treatment_code"

        claim_info["treatment_code"] = treatment_code
